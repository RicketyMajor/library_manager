...
class LibraryMainScreen(Screen):
    ...
    def action_add_book(self) -> None:
        ...
        def handle_menu_choice(choice: str | None) -> None:
            if choice == "scan":
                ...
                self.app.push_screen(ScannerModal())
            elif choice == "isbn":
                self.app.push_screen(IsbnModal(), self.handle_isbn_input)
            elif choice == "manual":
                self.app.push_screen(
                    ManualAddModal(), self.handle_manual_input)
    ...
    def handle_manual_input(self, payload: dict | None) -> None:
        if payload and payload.get("title"):  # Validación básica
            ...
            self.app.notify("El Título de la obra es obligatorio.",
                            severity="error")
    ...
    def action_edit_book(self) -> None:
        ...
        try:
            ...
            self.app.notify("Selecciona un libro en la tabla primero.",
                            severity="warning")
    ...
    def open_edit_modal_sync(self, book: dict, book_id: str) -> None:
        def save_changes(payload: dict | None) -> None:
            if payload:
                self.app.notify("Guardando cambios en el servidor...")
                self.process_edit(book_id, payload)
        self.app.push_screen(FullEditModal(book), save_changes)
    ...
    def process_edit(self, book_id: str, payload: dict) -> None:
        try:
            ...
            if resp.status_code == 200:
                self.app.call_from_thread(
                    self.app.notify, "¡Ficha actualizada!", title="Éxito")
                self.app.call_from_thread(self.load_all_data)
            else:
                self.app.call_from_thread(
    ...
    def action_lend_book(self) -> None:
        ...
        try:
            ...
            if row_key:
                ...
                self.app.push_screen(LendModal(), do_lend)
        except Exception:
            self.app.notify("Selecciona un libro.", severity="warning")
    ...
    def action_return_book(self) -> None:
        ...
        try:
            ...
            self.app.notify("Selecciona un préstamo activo.",
                            severity="warning")
...
# ================= INBOX =================
    def action_process_inbox(self) -> None:
        ...
        try:
            ...
            if row_key:
                ...
                self.app.notify(f"Procesando ISBN: {isbn}...")
                self.process_inbox_item(row_key, isbn)
        except Exception:
            self.app.notify("Selecciona un escaneo de la tabla.",
                            severity="warning")
    ...
    def action_log_pages(self) -> None:
        ...
        self.app.push_screen(LogPagesModal(), do_log)
    ...
    def action_finish_book(self) -> None:
        ...
        self.app.push_screen(FinishBookModal(), do_finish)
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if url:
                    ...
                    webbrowser.open(url)
                    self.app.call_from_thread(
                        self.app.notify, "Abriendo enlace en tu navegador...")
                else:
                    self.app.call_from_thread(
                        self.app.notify, "No hay enlace disponible.")
        except Exception as e:
            self.app.call_from_thread(
                self.app.notify, f"Error: {e}", severity="error")
...
# ================= MOTOR DE ELIMINACIÓN GLOBAL =================
    ...
    def action_delete_book(self) -> None:
        ...
        try:
            ...
            self.app.notify("Selecciona un libro primero.", severity="warning")
...
# ================= TRANSFERENCIA ENTRE DIRECTORIOS =================
    def action_move_book(self) -> None:
        ...
        try:
            ...
            self.app.notify("Selecciona un libro en la tabla.",
                            severity="warning")
    ...
    def process_move_book(self, book_id: str, dest_dir: int | None) -> None:
        try:
            ...
            if resp.status_code == 200:
                self.app.call_from_thread(
                    self.app.notify, "¡Libro transferido de carpeta!", title="Éxito")
